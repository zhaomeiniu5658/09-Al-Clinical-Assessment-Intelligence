    ...
    <div class="detail-page">
      ...
      <template v-else-if="task">
        ...
        <section class="detail-card result-card">
          ...
          <el-alert v-if="task.error_message" class="detail-error" type="error" :closable="false" show-icon>
            <template #title>任务异常：{{ task.error_message }}</template>
          </el-alert>
          <el-table class="qc-table" :data="qcItemRows" border>
            ...
            <el-table-column label="复核得分" width="110" align="center">
              <template #default="{ row }">{{ formatScore(row.review_score ?? null) }}</template>
            </el-table-column>
            <el-table-column prop="review_opinion" label="复核意见" min-width="220">
              <template #default="{ row }">
                <span class="review-opinion-text">{{ row.review_opinion || '-' }}</span>
              </template>
            </el-table-column>
            <el-table-column label="状态" width="100" align="center">
              <template #default="{ row }">
                <span class="review-badge" :class="isRowReviewed(row) ? 'reviewed' : 'unreviewed'">
                  {{ isRowReviewed(row) ? '已复核' : '未复核' }}
                </span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="120" align="center" fixed="right">
              <template #default="{ row, $index }">
                <el-button class="review-trigger" size="small" type="primary" link @click="openReviewDialog(row, $index)">
                  复核
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-dialog v-model="reviewDialogVisible" title="人工复核" width="460px" @closed="reviewFormRef?.clearValidate()">
          <p class="review-item-name">{{ selectedReviewRow?.hamd_item }}</p>
          <el-form ref="reviewFormRef" class="review-form compact-review-form" :model="reviewForm" :rules="reviewRules" label-position="top">
            <el-form-item label="复核评分" prop="reviewed_score">
              <el-input-number v-model="reviewForm.reviewed_score" :min="0" :max="100" :precision="1" class="full-width" />
            </el-form-item>
            <el-form-item label="复核原因" prop="review_reason">
              <el-input v-model="reviewForm.review_reason" type="textarea" :rows="4" placeholder="请填写复核原因" />
            </el-form-item>
          </el-form>
          <template #footer>
            <el-button @click="reviewDialogVisible = false">取消</el-button>
            <el-button type="primary" :loading="reviewing" @click="submitReview">提交复核</el-button>
          </template>
        </el-dialog>
...
import { ArrowLeft, DataAnalysis, DocumentChecked, Headset, Microphone } from '@element-plus/icons-vue'
import type { FormInstance, FormRules } from 'element-plus'
import { ElMessage } from 'element-plus'
import { computed, onBeforeUnmount, onMounted, reactive, ref } from 'vue'
import { useRoute } from 'vue-router'
import { apiClient } from '../api/client'
import { fetchTask, reviewTaskItem } from '../api/tasks'
...
const selectedReviewRow = ref<QcItemResult | null>(null)
const selectedReviewIndex = ref(-1)
const reviewFormRef = ref<FormInstance>()
const reviewForm = reactive({ reviewed_score: 0, review_reason: '' })
const reviewRules: FormRules = { reviewed_score: [{ required: true, message: '请输入复核评分', trigger: 'blur' }], review_reason: [{ required: true, message: '请填写复核原因', trigger: 'blur' }] }
const hamd17Items = [
  '1.抑郁情绪（悲伤、无望、无助、无价值）',
  '2.有罪感',
  '3.自杀',
...
function openReviewDialog(row: QcItemResult, index: number) {
  selectedReviewRow.value = row
  selectedReviewIndex.value = index
  reviewForm.reviewed_score = row.review_score ?? row.ai_score ?? row.doctor_score ?? 0
  reviewForm.review_reason = row.review_opinion ?? ''
  reviewDialogVisible.value = true
}
function isRowReviewed(row: QcItemResult) {
  return row.review_score !== null && row.review_score !== undefined
}
async function submitReview() {
  if (!task.value || selectedReviewIndex.value < 0) return
  await reviewFormRef.value?.validate()
  reviewing.value = true
  try {
    task.value = await reviewTaskItem(task.value.id, selectedReviewIndex.value, {
      review_score: reviewForm.reviewed_score,
      review_opinion: reviewForm.review_reason,
    })
...
.review-opinion-text { color: #68758b; line-height: 1.7; white-space: pre-wrap; word-break: break-word; }
</style>
