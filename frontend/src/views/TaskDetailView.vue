  ...
  <AppShell title="任务详情" :subtitle="task ? `任务编号 ${formatTaskCode(task)}` : '查看临床评估任务的质控结果'">
    <div class="detail-page">
      ...
      <template v-else-if="task">
        ...
        <section class="detail-card transcript-card">
          <div class="section-heading"><el-icon><Document /></el-icon><h2>转录文本</h2><span class="section-caption">ASR 识别结果</span></div>
          <p class="transcript-content">{{ task.asr_text || '暂无转录文本' }}</p>
        </section>
...
import { ArrowLeft, Checked, DataAnalysis, Document, DocumentChecked, Headset, Microphone } from '@element-plus/icons-vue'
...
@media (max-width: 580px) { .overview-card, .audio-card, .result-card, .transcript-card, .review-card { padding: 18px; }.overview-items, .result-grid, .review-record { grid-template-columns: 1fr; }.review-record > div:nth-child(n+3) { grid-column: auto; }.audio-file strong { max-width: 210px; } }
</style>
